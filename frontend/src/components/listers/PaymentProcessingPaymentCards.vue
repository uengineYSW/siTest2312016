    ...
    <div>
        ...
        >
            <v-row>
                <v-list-item class="d-flex">
                    ...
                    <div class="secondary-text-color" style="margin-left:30px;">결제 정보를 관리하는 주요 도메인</div>
                </v-list-item>
            </v-row>
        </v-card>
        <v-col style="margin-bottom:40px;">
            <div class="text-center">
                ...
                >
                    <PaymentProcessingPayment :offline="offline" class="video-card" :isNew="true" :editMode="true" v-model="newValue" 
        ...
        <v-row>
            <PaymentProcessingPayment :offline="offline" class="video-card" v-for="(value, index) in values" v-model="values[index]" v-bind:key="index" @delete="remove"/>
        </v-row>
    </div>
</template>
...
<script>
    ...
    import PaymentProcessingPayment from './../PaymentProcessingPayment.vue';
